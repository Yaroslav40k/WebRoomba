...
class InputServiceImplTest {
    ...
    @Mock private ObjectToJsonSerializerImpl serializer;
    ...
    void saveInput_withValidConfiguration_expectingCreatedInput(String serializedInput) {
        ...
        when(repository.save(any(InputEntity.class))).then(AdditionalAnswers.returnsFirstArg());
        ...
        verify(repository, times(1)).save(any(InputEntity.class));
    }

    @Test
    void getAllInputs_expectingListOfInputs () {
        when(repository.findAll()).thenReturn(List.of(new InputEntity()));
